// Lógica de negocio de la expulsión de un miembro, separada del envoltorio RPC
// (remove-guild-member.ts). El envoltorio solo resuelve la sesión y delega aquí;
// esta función recibe el `requesterId` ya autenticado y el `data` ya validado,
// así que es invocable directamente en tests con `#/db` mockeado — sin depender
// del transform del plugin de TanStack Start, que no está activo bajo Vitest y
// hace que un server fn llamado directo resuelva a `undefined`.
//
// El comportamiento (checks de autorización, mensajes de error, transacción con
// bloqueo, reverificación y limpieza en cascada) es idéntico al que tenía inline
// en el handler.
...
import type { RemoveGuildMemberValues } from '../schemas/guild-schemas'
import { resolveGuildBySlugOrThrow } from './resolve-guild-or-throw'

export async function removeGuildMemberHandler(
  data: RemoveGuildMemberValues,
  requesterId: string,
) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const lockedTarget = lockedMemberships.find((m) => m.userId === data.userId)
    ...
    await tx
      ...
      .where(
        and(eq(quests.guildId, guild.id), eq(quests.supervisorId, data.userId)),
      )
